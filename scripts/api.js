...
function handleRequestWithRetry(requestFn, options, callbackData, callbacks) {
    try {
        ...
        sys.logs.info("[jira] Handling request...: "+ JSON.stringify(error));
    }
}
...
exports.get = function(path, httpOptions, callbackData, callbacks) {
    ...
    return httpService.get(Jira(options), callbackData, callbacks);
};
...
exports.post = function(path, httpOptions, callbackData, callbacks) {
    let options = checkHttpOptions(path, httpOptions);
    return httpService.post(Jira(options), callbackData, callbacks);
};
...
exports.put = function(path, httpOptions, callbackData, callbacks) {
    let options = checkHttpOptions(path, httpOptions);
    return httpService.put(Jira(options), callbackData, callbacks);
};
...
exports.patch = function(path, httpOptions, callbackData, callbacks) {
    let options = checkHttpOptions(path, httpOptions);
    return httpService.patch(Jira(options), callbackData, callbacks);
};
...
exports.delete = function(path, httpOptions, callbackData, callbacks) {
    let options = checkHttpOptions(path, httpOptions);
    return httpService.delete(Jira(options), callbackData, callbacks);
};
...
exports.head = function(path, httpOptions, callbackData, callbacks) {
    let options = checkHttpOptions(path, httpOptions);
    return httpService.head(Jira(options), callbackData, callbacks);
};
...
exports.options = function(path, httpOptions, callbackData, callbacks) {
    let options = checkHttpOptions(path, httpOptions);
    return httpService.options(Jira(options), callbackData, callbacks);
};
...
exports.utils = {
    ...
    getConfiguration: function (property) {
        if (!property) {
            sys.logs.debug('[jira] Get configuration');
            return JSON.stringify(config.get());
        }
        sys.logs.debug('[jira] Get property: '+property);
...
function setApiUri(options) {
    let API_URL = config.get("JIRA_API_BASE_URL") + "/rest/api/2"
    let url = options.path || "";
    options.url = API_URL + url;
    sys.logs.debug('[jira] Set url: ' + options.path + "->" + options.url);
    return options;
}
...
function setAuthorization(options) {
    sys.logs.debug('[jira] Setting header authorization');
